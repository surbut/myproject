...
To fit for multiple K, we perform the floowing and plot the BF to determine the number of K that maximizes the BF:

```{r,eval=T}
fits <- lapply(8:12, function(k) FitGoM(t(b),K=k, tol=1))## try over different Ks
#fit=FitGoM(t(b),K=4, tol=1)
plot((8:12),sapply(fits,function(x){
  logBF=log(x$fit$BF)
return(logBF)}),main="logBFvsK",xlab="K",ylab="logBF")

```

We can see that the algorithm appears to stablize at K=10.
```{r}
...
StructureGGplot(omega = omega,
                ...
                palette = c(brewer.pal(8, "Accent"),brewer.pal(8, "Dark2")),
...
plot(d^2/sum(d^2),main="Proportion of Variation Explained",xlab="Number of Eigenvalues")
