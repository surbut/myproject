...
We can see that the barcodes are in the rows and the tissues are in the columns, and that the dimensions are as follow:
...
StructureGGplot(omega = omega,
                ...
                axis_tick = list(axis_ticks_length = .1,
                                 ...
                                 axis_ticks_lwd_x = .1,
                                 axis_label_size = 7,
                                 axis_label_face = "bold"),yaxis_label = "Clone_Number",figure_title = "Clonal Membership in Clusters")
```


We extract the top clones driving each cluster using the ExtractTopFeatures functionality of the CountClust package. We first perform the cluster annotations from the GoM model fit with $k=10` on the clone data. We can see that some of the clusters have strong  lung expression, and some have more tissue specific expression in adrenal, left ovary, liver or right ovary. 
